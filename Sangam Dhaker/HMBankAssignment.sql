...
WHERE   
    customer_id IN (SELECT customer_id   
                    FROM Customers   
                    WHERE (SELECT COUNT(*) FROM Accounts WHERE Accounts.customer_id = Customers.customer_id) > 1);
...
FROM   
    Customers   
WHERE   
    customer_id NOT IN (SELECT DISTINCT customer_id FROM Transactions);
...
FROM   
    Transactions   
WHERE   
    account_id = (SELECT TOP 1 account_id   
                  FROM Accounts   
...
FROM   
    Transactions   
WHERE   
    account_id IN (SELECT account_id FROM Accounts WHERE customer_id = 5); 
